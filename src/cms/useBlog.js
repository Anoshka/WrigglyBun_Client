import { useEffect, useMemo, useState } from 'react'
import { sanity } from './sanityClient'
import { urlFor } from './imageUrl'
import { blogPostsQuery, blogPostBySlugQuery } from './queries'

function toImgUrl(img, width = 1200) {
  return img ? urlFor(img).width(width).auto('format').url() : null
}

export function useBlogPosts() {
  const [raw, setRaw] = useState(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  useEffect(() => {
    setLoading(true)
    sanity.fetch(blogPostsQuery).then(setRaw).catch(setError).finally(() => setLoading(false))
  }, [])

  const posts = useMemo(() => {
    if (!raw) return []
    return (raw || []).map((p) => ({
      ...p,
      image: p.thumbnail ? toImgUrl(p.thumbnail, 800) : null,
    }))
  }, [raw])

  return { posts, loading, error }
}

export function useBlogPost(slug) {
  const [raw, setRaw] = useState(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  useEffect(() => {
    if (!slug) return
    setLoading(true)
    sanity.fetch(blogPostBySlugQuery, { slug }).then(setRaw).catch(setError).finally(() => setLoading(false))
  }, [slug])

  const post = useMemo(() => {
    if (!raw) return null
    return {
      ...raw,
      thumbnail: raw.thumbnail ? toImgUrl(raw.thumbnail, 1200) : null,
      images: (raw.images || []).map((img) => toImgUrl(img, 1200)),
    }
  }, [raw])

  return { post, loading, error }
}
